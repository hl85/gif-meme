...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await provider.searchStickers(query, page, perPage, customerId);
    return NextResponse.json(data);
  } catch (error) {
    console.error('Klipy API error:', error);
    return NextResponse.json({ error: 'Failed to fetch sticker search results' }, { status: 500 });
  }
}
